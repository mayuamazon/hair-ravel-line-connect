...
export function buildThankYouFlex({ salonName, name, visitDate, services, careUrl, reviewUrl, products }) {
  ...
  if (reviewUrl) footer.push(uriButton('Googleマップで口コミを書く', reviewUrl, 'secondary'));
  return {
    type: 'flex',
...
// ---------- 本日のご予約・来店一覧 → オーナー通知（管理画面ボタン用） ----------
export function ownerTodayListText({ dateLabel, bookings }) {
  if (!bookings.length) {
    return `📋 本日のご予約・来店（${dateLabel}）\n\n本日のご予約はありません。`;
  }
  const lines = bookings.map(b =>
    `${b.confirmed_time || b.preferred_time || '--:--'}　${b.name}（${(b.services || []).join(' / ') || '—'}）`
    + (b.status === 'confirmed' ? '✅' : '⏳承認待ち')
    + (b.line_user_id ? ' 💬LINE' : ''));
  return [`📋 本日のご予約・来店（${dateLabel}）`, '', ...lines, '', `計${bookings.length}件`].join('\n');
}
