...
export async function handleEvents(events, ctx) {
  ...
  // ON/OFFトグル：welcome/subscribe_reply/accept_reply の「返信文だけ」を抑制する。
  // 状態変更（購読登録・予約確定）は常に実行する。取得失敗時は全on扱い（現行挙動）。
  let toggles = {};
  try { toggles = typeof store.getLineToggles === 'function' ? await store.getLineToggles() : {}; }
  catch { toggles = {}; }
  const replyAllowed = key => toggles[key] !== false; // 既定on=true
  const skipLog = key => store.appendLog(`〔${key}〕はオフのため送信スキップ`).catch(() => {});
  const handled = [];
  for (const ev of events || []) {
    const uid = ev.source?.userId || '';
    try {
      if (ev.type === 'follow') {
        if (ev.replyToken && replyAllowed('welcome')) {
          ...
          if (ev.replyToken) await skipLog('welcome');
          await store.appendLog(`webhook follow ${store.maskId(uid)}`);
        }
        handled.push('follow');
        continue;
      }
      ...
      if (ev.type === 'postback') {
        ...
        if (pa === 'booking_accept' || pa === 'booking_repick') {
          const handledTag = await handleProposalReply(ev, pd.get('id') || '', pa, ctx, notifyOwner, { replyAllowed, skipLog });
      ...
      if (action && uid) {
        await store.setSubscriber(uid, action === 'on'); // 状態変更は常に実行
        if (ev.replyToken) {
          if (replyAllowed('subscribe_reply')) {
            await line.reply(ev.replyToken, [{ type: 'text', text: action === 'on' ? subscribeOnText : subscribeOffText }]);
          } else {
            await skipLog('subscribe_reply');
          }
        }
...
// gate.replyAllowed/skipLog でトグルOFF時に「返信文だけ」を抑制する（状態変更は常に実行）。
async function handleProposalReply(ev, id, action, ctx, notifyOwner, gate = {}) {
  const { store, line, config } = ctx;
  const replyAllowed = gate.replyAllowed || (() => true);
  const skipLog = gate.skipLog || (() => Promise.resolve());
  ...
  if (action === 'booking_accept') {
    ...
    if (ev.replyToken) {
      if (replyAllowed('accept_reply')) {
        ...
        await skipLog('accept_reply');
      }
    }
    if (replyAllowed('owner_response')) await notifyOwner(ownerAcceptedText(updated));
    else await skipLog('owner_response');
