...
const FIELDS = [
  ...
  'nakamuraUserId', 'matsuyoshiUserId',
...
export function createSecretStore({ configDir, passphrase } = {}) {
  ...
  function envSettings(env = process.env) {
    ...
    const out = {
      ...
      nakamuraUserId: pick('LINE_NAKAMURA_USER_ID'),
      matsuyoshiUserId: pick('LINE_MATSUYOSHI_USER_ID'),
  ...
  function masked(settings) {
    ...
    return {
      ...
      nakamuraUserId: last4(settings.nakamuraUserId),
      matsuyoshiUserId: last4(settings.matsuyoshiUserId),
